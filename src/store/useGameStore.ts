...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  autoBlackMove: () => {
    ...
    if (solutionStep < puzzle.solution.length) {
      ...
      const piece = getPieceAt(
        board,
        expectedMove.fromRow,
        expectedMove.fromCol,
      );
      if (
        piece &&
        piece.color === "black" &&
        isValidMove(board, piece, expectedMove.toRow, expectedMove.toCol)
      ) {
  ...
  undoMove: () => {
    const state = get();
    const { moveHistory, currentPuzzleId, isGameOver } = state;
    ...
    if (!isGameOver) {
      ...
      if (lastMove && lastMove.piece.color === "black") {
        undoCount = 2;
      }
    }
